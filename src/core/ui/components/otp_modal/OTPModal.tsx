...
export interface OTPModalProps extends ModalProps {
  title?: string;
  message?: string;
  description?: string;
  error?: null | {
...
export const OTPModal = ({
  ...
  return (
    <Modal open={open} onClose={handleCloseTimer}>
      <div
        className={clsx(
      ...
      >
        ...
        >
          ...
          <div
            className={clsx(
              "grid grid-cols-[auto_auto_auto_auto_auto_auto] justify-center justify-items-center gap-x-[1.5rem]"
            )}
          >
            {pinLength.map((item) => (
              <div
                key={item}
                className={clsx(
                  "grid place-content-center place-items-center z-9999",
                  "bg-dark-charcoal",
        ...
        {timeLeft <= 0 && (
          <Button onClick={handleClickSendOTP}>{cta.primary.children}</Button>
        )}
      </div>
    </Modal>
  );
