...
export const DashboardCustomerHome = () => {
  ...
  const customer = appState.user.customer;
  const initial = !customer || !customer.name ? "" : customer.name.charAt(0);
  const photo = !customer || !customer.photo ? "" : customer.photo ?? "";
  const menu =
        ...
        {menu.map((item, index) => {
          ...
          return (
            <Link
              ...
              to={
                item.id === "scan_record"
                  ? href.replace(":id", appState.cases.data.selected?.id ?? "")
                  : href
              }
              className={clsx(
            ...
            >
              ...
              >
                ...
                >
                  <p
                    className={clsx("text-[1.125rem] font-bold text-[#FAFAFA]")}
                  >
                    {item.name}
                  </p>
