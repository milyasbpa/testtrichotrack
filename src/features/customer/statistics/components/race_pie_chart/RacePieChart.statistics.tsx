...
export interface RacePieChartCustomerStatisticsProps {
  data?: number[];
  labels?: string[];
}

export const RacePieChartCustomerStatistics = ({
  data = [],
  labels = [],
}: RacePieChartCustomerStatisticsProps) => {
  const chartData = {
    labels: labels,
    datasets: [
      {
  ...
  return (
    <div className={clsx("w-full")}>
      <div
        className={clsx(
          "grid grid-cols-4 w-full place-content-center place-items-center",
      ...
      >
        <div className={clsx("w-[75%]")}>
          <Pie id={"race-chart"} options={options} data={chartData} />
        </div>
      </div>
    </div>
  );
