...
export interface CategoryRadarChartDiagnosisProps {
  data: {
    id: string;
    label: {
      name: string;
...
export const CategoryRadarChartDiagnosis = ({
  data = [],
  // labels = [],
  onClick = () => {},
}: CategoryRadarChartDiagnosisProps) => {
  const dataMap = {
    labels: data.map(() => ""),
    datasets: [
      {
  ...
  return (
    <div
      className={clsx(
        "grid justify-center justify-items-center grid-cols-1",
        "w-full",
        "max-h-[576px]",
        "relative"
      )}
