...
export const CustomerLogoutContainer = () => {
  ...
  const handleClickConfirm = async (_: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const staffToken = cookies.get("staff-token");
    await cookies.set("token", staffToken, { path: "/" });
    await dispatchApp({
      type: AppActionEnum.SetAuthData,
      payload: {
        ...appState.auth,
        role:
          appState.user.staff?.permission === "Admin"
            ? "ADMIN"
            : appState.user.staff?.permission === "Manager"
            ? "MANAGER"
            : "EMPLOYEE",
      },
    });
